...
export default function NotificationsScreen({ navigation }: any) {
  ...
  const handlePress = (notification: AppNotification) => {
    if (notification.type === 'prontuario_gerado' && notification.document) {
      navigation.navigate('DocumentDetail', { document: notification.document });
      return;
    }
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: isDark ? '#15171a' : '#fcfcfb' }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: primaryTeal }]}>Notificacoes</Text>
...
const styles = StyleSheet.create({
  ...
  unreadDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: '#00ccdb' },
