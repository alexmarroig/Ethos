<<<<<<< HEAD:apps/ethos-mobile/src/screens/SessionHubScreen.tsx
...
>>>>>>> 97f19340c110e556bf5c1ebe71a5b625f605e9e4:apps/ethos-mobile/src/features/sessions/screens/SessionHubScreen.tsx
...
const buildDraftContent = ({
  ...
  `SessÃ£o: ${sessionTime}`,
  '',
  'Rascunho inicial gerado a partir da transcriÃ§Ã£o:',
  rawText,
  '',
...
export default function SessionHubScreen({ navigation, route }: any) {
  ...
  const sessionTime = route?.params?.time || (session?.scheduled_at ? new Date(session.scheduled_at).toLocaleString('pt-BR') : 'SessÃ£o em andamento');
=======
...
>>>>>>> 97f19340c110e556bf5c1ebe71a5b625f605e9e4:apps/ethos-mobile/src/features/sessions/screens/SessionHubScreen.tsx
  ...
  const waitForCompletedJob = async (jobId: string) => {
    for (let attempt = 0; attempt < 12; attempt += 1) {
      ...
      if (job.status === 'failed') throw new Error('A transcriÃ§Ã£o falhou.');
      await wait(250);
    }

    throw new Error('A transcriÃ§Ã£o demorou mais do que o esperado.');
  };
  ...
  const handleStop = async () => {
    ...
    try {
      ...
      if (!session?.id) {
        Alert.alert('GravaÃ§Ã£o salva', uri ? 'A gravaÃ§Ã£o foi concluÃ­da localmente.' : 'A gravaÃ§Ã£o foi encerrada.');
        setDuration(0);
        return;
      }
      ...
      const rawText = `SessÃ£o registrada em ${sessionTime}. DuraÃ§Ã£o aproximada: ${formatTime(duration)}. Arquivo local: ${uri ?? 'nÃ£o disponÃ­vel'}.`;
      ...
      Alert.alert('Erro', error?.message ?? 'NÃ£o foi possÃ­vel gerar o rascunho clÃ­nico.');
    } finally {
      try {
        await Audio.setAudioModeAsync({
      ...
      <View style={styles.recorderBody}>
        ...
        {isProcessingDraft ? (
          <View style={styles.processingRow}>
            ...
            <Text style={styles.processingText}>TranscriÃ§Ã£o concluÃ­da, preparando nota...</Text>
          </View>
        ) : null}
      </View>
      ...
      <Animated.View entering={SlideInDown.duration(600)} style={styles.controlsContainer}>
        ...
        <View style={styles.micToggleContainer}>
          <View style={[styles.micToggle, { backgroundColor: '#272b34' }]}>
            ...
            <Text style={styles.micToggleText}>{session?.duration_minutes ? `${session.duration_minutes} min previstos` : 'SessÃ£o livre'}</Text>
          </View>
        </View>
      </Animated.View>
    </View>
