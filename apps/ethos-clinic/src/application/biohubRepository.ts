...
class SqlBiohubRepository implements BiohubRepository {
  ...
  async healthcheck() { await this.sql`SELECT 1`; }
  ...
  async listSubscriptions(userId: string) { return await this.sql`SELECT id,user_id,source,plan_code,status,current_period_start,current_period_end FROM biohub_subscriptions WHERE user_id=${userId}` as BiohubSubscription[]; }
  ...
  async createOverride(override: BiohubPlanOverride) { await this.sql`INSERT INTO biohub_plan_overrides (id,user_id,override_plan,reason,expires_at,set_by_admin_id,active,updated_at) VALUES (${override.id},${override.user_id},${override.override_plan},${override.reason},${override.expires_at ?? null},${override.set_by_admin_id},${override.active},NOW())`; }
  async deactivateOverrides(userId: string) { await this.sql`UPDATE biohub_plan_overrides SET active=false, updated_at=NOW() WHERE user_id=${userId} AND active=true`; }
  async addAudit(log: Omit<BiohubAccessAuditLog, "id" | "created_at">) { await this.sql`INSERT INTO biohub_access_audit_logs (id,actor_user_id,target_user_id,action_type,before_json,after_json,created_at,updated_at) VALUES (${uid()},${log.actor_user_id},${log.target_user_id},${log.action_type},${JSON.stringify(log.before_json)},${JSON.stringify(log.after_json)},NOW(),NOW())`; }
}
