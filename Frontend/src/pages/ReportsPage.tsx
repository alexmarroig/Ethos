...
const buildClinicalContext = (patient?: Patient) =>
  [
    ...
    patient?.has_psychiatric_followup
      ? `Acompanhamento psiquiátrico: ${patient.psychiatrist_name || "Em acompanhamento"}${patient.psychiatrist_contact ? ` (${patient.psychiatrist_contact})` : ""}`
      : null,
...
const buildStarter = (input: {
  ...
  const sessionBlock = input.sessions
    .map(
      (session, index) =>
        `Sessão ${index + 1} · ${formatDateTime(session.scheduled_at)}\nTipo: ${input.attendanceType}\nStatus: ${sessionStatusLabel(session.status)}\nTranscrição: ${session.has_transcription ? "Disponível" : "Não disponível"}`,
    )
    .join("\n\n");
  ...
  if (input.kind === "longitudinal_record") {
    return [
      ...
      "",
      "CONTEXTO CLÍNICO",
      buildClinicalContext(input.patient) || "Complementar com o contexto clínico do acompanhamento.",
      "",
      "SESSÕES DE REFERÊNCIA",
      sessionBlock || "Selecionar sessões para compor a evolução.",
      "",
  ...
  return [
    ...
    `Psicóloga responsável: ${input.psychologistName}`,
    `CRP: ${input.crp || ""}`,
    input.patient?.name ? `Paciente: ${input.patient.name}` : "",
    `Tipo de atendimento: ${input.attendanceType}`,
    input.sessions[0]?.scheduled_at ? `Sessão de referência: ${formatDateTime(input.sessions[0].scheduled_at)}` : "",
    "",
    "CONTEXTO CLÍNICO",
    buildClinicalContext(input.patient) || "Registrar contexto clínico relevante do atendimento.",
    "",
    "DESCRIÇÃO DA SESSÃO",
    "",
...
export default function ReportsPage() {
  ...
  const availableSessions = useMemo(
    () =>
      sessions
        .filter((session) => !editor.patient_id || session.patient_id === editor.patient_id)
  ...
  const selectedSessions = useMemo(() => {
    const ids =
      editor.kind === "longitudinal_record"
        ? editor.selected_session_ids
        : editor.session_id
          ? [editor.session_id]
          : [];
  ...
  const applyStarter = (partial?: Partial<ReportEditorState>) => {
    ...
    const sourceSessionIds =
      next.kind === "longitudinal_record"
        ? next.selected_session_ids
        : next.session_id
          ? [next.session_id]
          : [];
  ...
  const handleImportTranscript = async () => {
    const sourceSessionId =
      editor.kind === "longitudinal_record" ? editor.selected_session_ids[0] : editor.session_id;
    if (!sourceSessionId) {
      toast({
        title: "Selecione uma sessão",
        description: "Escolha uma sessão para usar a transcrição.",
        variant: "destructive",
      });
      return;
    }
    ...
    toast({
      ...
      description: "A transcrição foi trazida para a área de apoio.",
    });
  };
  ...
  const handleImproveWithAi = async (mode: "manual" | "transcript") => {
    ...
    if (!sourceText) {
      toast({
        ...
        description: "Escreva anotações ou use uma transcrição antes de acionar a IA.",
        variant: "destructive",
      });
      return;
    }
    ...
    toast({
      ...
      description: "A IA reorganizou o texto principal.",
    });
  };
  ...
  const contextCards = [
    ["Profissão", selectedPatient?.profession],
    ["Escolaridade", selectedPatient?.education_level],
    ["Estado civil", selectedPatient?.marital_status],
    ["Queixa principal", selectedPatient?.main_complaint],
    ["Objetivos terapêuticos", selectedPatient?.therapy_goals],
    ["Origem da demanda", selectedPatient?.referral_source],
    ["Modalidade do cuidado", selectedPatient?.care_interest],
    [
      "Psiquiatria",
      selectedPatient?.has_psychiatric_followup
        ? `${selectedPatient.psychiatrist_name || "Em acompanhamento"}${selectedPatient.psychiatrist_contact ? ` · ${selectedPatient.psychiatrist_contact}` : ""}`
        : undefined,
    ],
    ["Medicações", selectedPatient?.psychiatric_medications],
    ["Técnicas recorrentes", selectedPatient?.recurring_techniques],
    ["Indicação documental", selectedPatient?.report_indication],
    ["Observações para relatório", selectedPatient?.report_notes],
    ["Status do acompanhamento", selectedPatient?.care_status],
      ...
      <div className="content-container py-8 md:py-12">
        <motion.header className="mb-8" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="mt-2 text-muted-foreground">
            Relatórios profissionais com contexto clínico estruturado e edição mais legível.
          </p>
        </motion.header>
          ...
          <DialogContent className={`${isFullscreen ? "h-[96vh] max-w-[98vw]" : "h-[92vh] max-w-[min(96vw,1540px)]"} overflow-hidden border-border/70 bg-background p-0`}>
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <DialogDescription className="mt-2 text-sm text-muted-foreground">
                    Modo híbrido: contexto clínico, corpo em texto livre e preview de impressão.
                  </DialogDescription>
                </div>
                <div className="flex flex-wrap gap-2">
                  <Button variant="outline" size="sm" className="gap-2" onClick={() => setLayoutMode("split")}>
                    <Columns2 className="h-4 w-4" />
                    Lado a lado
                  </Button>
                  <Button variant="outline" size="sm" className="gap-2" onClick={() => setLayoutMode("editor")}>
                    <Wand2 className="h-4 w-4" />
                    Foco na edição
                  </Button>
                  <Button variant="outline" size="sm" className="gap-2" onClick={() => setLayoutMode("preview")}>
                    <Eye className="h-4 w-4" />
                    Foco no preview
                  </Button>
                  <Button variant="outline" size="sm" className="gap-2" onClick={() => setIsFullscreen((current) => !current)}>
                  ...
                  <div className="space-y-5">
                    <section className="rounded-[1.2rem] border border-border bg-card p-4">
                      ...
                      {loadingPatientContext ? (
                        <p className="mt-3 text-sm text-muted-foreground">Carregando contexto clínico...</p>
                      ) : contextCards.length === 0 ? (
                        <p className="mt-3 text-sm text-muted-foreground">Selecione um paciente para ver o contexto.</p>
                      ) : (
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        <Button variant="secondary" size="sm" onClick={() => appendToContent(buildClinicalContext(selectedPatient))} disabled={!selectedPatient}>
                          Inserir dados da ficha
                        </Button>
                      </div>
                    </section>
                    ...
                    <section className="rounded-[1.2rem] border border-border bg-card p-4">
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        {editor.kind === "longitudinal_record"
                          ? "Selecione várias sessões para a evolução."
                          : "Selecione a sessão principal."}
                      </p>
                      <div className="mt-4 space-y-3">
                        {editor.kind === "session_report" ? (
                          <select value={editor.session_id} onChange={(event) => setEditor((current) => ({ ...current, session_id: event.target.value }))} className="flex h-11 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm">
                            ...
                            {availableSessions.map((session) => (
                              <option key={session.id} value={session.id}>
                                {formatDateTime(session.scheduled_at)} · {sessionStatusLabel(session.status)}
                              </option>
                            ))}
                          </select>
                        ) : (
                          <div className="max-h-[240px] space-y-2 overflow-auto pr-1">
                            {availableSessions.map((session) => {
                              ...
                              return (
                                <label key={session.id} className="flex cursor-pointer items-start gap-3 rounded-xl border border-border/70 bg-background/70 p-3">
                                  <input
                                    type="checkbox"
                                    checked={checked}
                                    onChange={(event) =>
                                      setEditor((current) => ({
                                        ...current,
                                        selected_session_ids: event.target.checked
                                          ? [...current.selected_session_ids, session.id]
                                          : current.selected_session_ids.filter((id) => id !== session.id),
                                      }))
                                    }
                                  />
                                  <div>
                                    <p className="text-sm font-medium text-foreground">{formatDateTime(session.scheduled_at)}</p>
                                    <p className="text-xs text-muted-foreground">
                                      {sessionStatusLabel(session.status)} · {session.has_transcription ? "Transcrição disponível" : "Sem transcrição"}
                                    </p>
                                  </div>
                                </label>
                              );
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        <Button variant="outline" size="sm" onClick={() => appendToContent(selectedSessions.map((session) => `${formatDateTime(session.scheduled_at)} · ${sessionStatusLabel(session.status)}`).join("\n"))} disabled={selectedSessions.length === 0}>
                          Inserir resumo da sessão
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => void handleImportTranscript()}>
                          Usar transcrição
                        </Button>
                      </div>
                    </section>
                ...
                <section className={`${layoutMode === "split" ? "border-r" : ""} overflow-y-auto px-6 py-5`}>
                  <div className="space-y-5">
                    <section className="rounded-[1.2rem] border border-border bg-card p-4">
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">Defina paciente, tipo de peça e sessão de referência.</p>
                      <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                        <div className="space-y-2">
                          <label className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Paciente</label>
                          <select value={editor.patient_id} onChange={(event) => setEditor((current) => ({ ...current, patient_id: event.target.value, session_id: "", selected_session_ids: [] }))} className="flex h-11 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm">
                        ...
                        <div className="space-y-2">
                          <label className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Finalidade</label>
                          <select value={editor.purpose} onChange={(event) => setEditor((current) => ({ ...current, purpose: event.target.value }))} className="flex h-11 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm">
                        ...
                        <div className="space-y-2">
                          <label className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Modelo</label>
                          <select value={editor.kind} onChange={(event) => setEditor((current) => ({ ...current, kind: event.target.value as ReportKind, session_id: "", selected_session_ids: [] }))} className="flex h-11 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm">
                        ...
                        <div className="space-y-2">
                          <label className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Sessão vinculada</label>
                          <Button variant="outline" className="w-full justify-start text-left" onClick={() => applyStarter()}>
                            Gerar estrutura
                          </Button>
                        </div>
                      </div>
                      <div className="mt-4">
                        <label className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Tipo de atendimento</label>
                        <select value={editor.attendanceType} onChange={(event) => setEditor((current) => ({ ...current, attendanceType: event.target.value }))} className="mt-2 flex h-11 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm">
                    ...
                    <section className="rounded-[1.2rem] border border-border bg-card p-4">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-muted-foreground">Escreva ou revise o texto final do relatório.</p>
                        </div>
                        <div className="text-xs text-muted-foreground">
                          {(editor.content.match(/\n{2,}/g)?.length ?? 0) + 1} seção(ões)
                        </div>
                      </div>
                      <Textarea value={editor.content} onChange={(event) => setEditor((current) => ({ ...current, content: event.target.value }))} className="mt-4 min-h-[420px] resize-none rounded-2xl border-border/80 bg-background px-5 py-4 text-base leading-7" placeholder="Descreva o contexto clínico, evolução, intervenções e encaminhamentos com linguagem técnica e clara." />
                    </section>

                    <section className="rounded-[1.2rem] border border-border bg-card p-4">
                      <h3 className="font-serif text-lg text-foreground">Fonte para IA</h3>
                      <p className="mt-1 text-sm text-muted-foreground">Cole anotações clínicas ou carregue a transcrição de uma sessão.</p>
                      <Textarea value={editor.sourceText} onChange={(event) => setEditor((current) => ({ ...current, sourceText: event.target.value }))} className="mt-4 min-h-[180px] resize-none rounded-2xl border-border/80 bg-background px-5 py-4 text-sm leading-6" placeholder="Cole aqui suas anotações clínicas ou use a transcrição como apoio para a IA." />
                      <div className="mt-4 flex flex-wrap gap-2">
                        <Button variant="outline" size="sm" onClick={() => void handleImportTranscript()}>
                          Usar transcrição do áudio
                        </Button>
                        <Button variant="outline" size="sm" className="gap-2" onClick={() => void handleImproveWithAi("manual")} disabled={improving}>
                          {improving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4 text-sky-500" />}
                          Melhorar com IA
                        </Button>
                        <Button variant="outline" size="sm" className="gap-2" onClick={() => void handleImproveWithAi("transcript")} disabled={improving}>
                          {improving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4 text-sky-500" />}
            ...
            <DialogFooter className="border-t border-border/70 bg-card px-6 py-4">
              <div className="flex flex-wrap gap-2">
                ...
                <Button variant="outline" className="gap-2" onClick={() => downloadReportDoc(previewHtml, `${selectedPatient?.name ?? "relatorio"}.doc`)}>
              ...
              <div className="flex flex-wrap gap-2">
                {selectedReportId ? (
                  <ShareWithPatientButton type="reports" id={selectedReportId} shared={(reports.find((item) => item.id === selectedReportId) as unknown as { shared_with_patient?: boolean })?.shared_with_patient ?? false} />
                ) : null}
                <Button variant="outline" onClick={() => setDialogOpen(false)}>
                  Fechar
                </Button>
                <Button variant="secondary" onClick={() => void handleSave(false)} disabled={saving || !editor.patient_id || !editor.content.trim()}>
                  {saving ? "Salvando..." : "Salvar rascunho"}
                </Button>
                <Button onClick={() => void handleSave(true)} disabled={saving || !editor.patient_id || !editor.content.trim()}>
