...
const Index = () => {
  ...
  useEffect(() => {
    if (!isAuthenticated || showLogin || !user?.role) {
      return;
    }

    const loaders = prefetchByRole[user.role as keyof typeof prefetchByRole] ?? [importHomePage];
    const prefetch = () => {
  ...
  }, [isAuthenticated, showLogin, user?.role]);
