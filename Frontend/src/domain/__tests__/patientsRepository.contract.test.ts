...
import { getPatientsRepository, setPatientsRepositoryForTests } from "@/domain/repositories/patientsRepository";
...
describe("PatientsRepository contract", () => {
  ...
  it("allows overriding repository implementation", async () => {
    const override = {
      list: vi.fn().mockResolvedValue({ success: true as const, data: [{ id: "fake" }], request_id: "override" }),
      getById: vi.fn(),
      create: vi.fn(),
      update: vi.fn(),
      grantAccess: vi.fn(),
